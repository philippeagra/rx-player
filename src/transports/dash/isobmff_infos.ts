...
import { getISOBMFFMdatBatch } from "../../parsers/containers/isobmff/utils";
...
function getISOBMFFTimingInfos(
  ...
  // In that mode, we should only refer to ISOBMFF infos to get time and duration.
  if (lowLatencyMode) {
    if (baseDecodeTime >= 0) {
...
/**
 * Get low latency batches (various boxes + mdat)
 * @param {Object} record
 * @param {Uint8Array} data
 * @param {string} contentId
 * @returns {Array.<Uint8Array>}
 */
export function getISOBMFFLowLatencyChunks(
  record: IChunkRecord,
  data: Uint8Array,
  contentId: string
): Uint8Array[] {
  const chunkRecord = record[contentId] || {
    totalChunk: new Uint8Array(0),
    parsePosition: 0,
  };
  const { totalChunk, parsePosition } = chunkRecord;

  const newTotalChunk = new Uint8Array(
    totalChunk.length +
    data.length
  );

  newTotalChunk.set(totalChunk);
  newTotalChunk.set(data, totalChunk.length);

  const { chunks, position: newPosition } =
    getISOBMFFMdatBatch(newTotalChunk, parsePosition);

  chunkRecord.totalChunk = newTotalChunk;
  chunkRecord.parsePosition = newPosition;
  record[contentId] = chunkRecord;
